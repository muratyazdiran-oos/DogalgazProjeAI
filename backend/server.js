...
const artifactUpload=multer({dest:uploadDir,limits:{fileSize:maxArtifactMB*1024*1024,files:1,fields:4,fieldSize:8192,parts:6}});
...
const prompt=`Bir doğalgaz tesisatı keşif videosunu analiz et ve yalnızca videoda görülebilen verilere dayalı 2B proje TASLAĞI üret. Sayaç, kombi, ocak, vana, menfez, görülebilen borular ve oda sınırlarını tespit et. Koordinatlar 0..1 normalize olsun. Kombi/ocak etiketinde marka veya model okunuyorsa brand/model alanlarına yaz; emin değilsen null kullan. Model için en fazla 3 olası aday üret ve her adayda confidence ver. Logo/etiket görünmüyorsa marka/model UYDURMA. capacityKW yalnız etiketten güvenilir okunuyorsa doldur; model adına bakıp tahmin etme. Her boru ve cihaz için aiConfidence ve requiresReview üret. Her cihaz için mümkünse cihazın en net görüldüğü videoTimeSeconds zamanını ve o karedeki 0..1 normalize videoBoundingBox alanını ver; güvenilir değilse null kullan. Ölçek/LiDAR verisi videoda kesin değilse boru lengthMeters için tahmin yapma; 0 kullan. AI marka/model önerisi mühendis veya kullanıcı onayı olmadan resmî hesapta kesin kabul edilmemelidir. Bu veri resmî proje değildir; mühendis kontrolü gerekir.`;

app.get("/health",(_,res)=>res.json({ok:true,version:apiVersion,model,maxVideoMB,maxArtifactMB,artifactStorageMode,authRequired:Boolean(appApiToken),teamAuthEnabled:Boolean(teamAuthSecret),postgres:Boolean(pool)}));
...
app.get("/v1/teams/:id/dashboard",requireTeamAuth,async(req,res)=>{
  ...
  const {rows:projects}=await pool.query(`
    SELECT p.id,p.version,p.owner_email,p.updated_at,
      COALESCE(p.project->>'name','Proje') AS name,
      COALESCE(p.project->'projectWorkflow'->>'status','draft') AS status,
      p.project->'projectWorkflow'->>'assignedToEmail' AS assigned_to_email,
      p.project->'projectWorkflow'->>'dueDate' AS due_date,
      COALESCE(p.project->'approvalWorkflow'->>'status','draft') AS approval_status,
      CASE
        WHEN COALESCE(p.project->'projectWorkflow'->>'status','draft') <> 'completed'
         AND NULLIF(p.project->'projectWorkflow'->>'dueDate','') IS NOT NULL
         AND NULLIF(p.project->'projectWorkflow'->>'dueDate','')::timestamptz < now()
        THEN true ELSE false END AS overdue,
      (SELECT count(*)::int FROM project_artifacts a WHERE a.project_id=p.id) AS artifact_count,
      CASE WHEN p.project->'arRoomAlignment' IS NOT NULL THEN true ELSE false END AS has_ar_alignment
    FROM projects p WHERE p.team_id=$1 ORDER BY p.updated_at DESC LIMIT 100
  `,[teamID]);
  ...
  const approvalQueueCount=projects.filter(p=>p.approval_status!=="approved" && ["engineeringReview","fieldVisit"].includes(p.status)).length;
  res.json({teamName:teams[0].name,members,projects:projects.map(p=>({id:p.id,name:p.name,version:p.version,updatedAt:p.updated_at,ownerEmail:p.owner_email,status:p.status,assignedToEmail:p.assigned_to_email,dueDate:p.due_date,approvalStatus:p.approval_status,overdue:p.overdue,artifactCount:p.artifact_count,hasARAlignment:p.has_ar_alignment})),countsByRole,countsByStatus,overdueCount,approvalQueueCount});
});
